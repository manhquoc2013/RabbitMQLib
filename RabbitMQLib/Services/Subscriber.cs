...
using RabbitMQSubcriber.Common.Constants;
...
#nullable disable
namespace RabbitMQSubcriber.Services
{
        ...
        private void CreateExchangeOrQueue()
        {
            ...
            catch (Exception ex)
            {
                ...
                if (_options.RecreateIfFailed)
                {
                    ...
                    if (_options.ExchangeOrQueue == ExchangeOrQueueEnum.Exchange && !string.IsNullOrWhiteSpace(_options.QueueName))
                    {
